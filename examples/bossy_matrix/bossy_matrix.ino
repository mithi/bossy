...
void update_stick_display(const struct Stick stick) {
  uint8_t state = bossy.savedState(stick.id);
  
  uint8_t color1 = stick_first_colors[state];
  uint8_t color2 = stick_second_colors[state];

  uint8_t x1 = stick.first_pixel[Xi];
  uint8_t y1 = stick.first_pixel[Yi];     

  uint8_t x2 = stick.second_pixel[Xi];
  uint8_t y2 = stick.second_pixel[Yi];
...
void update_switch_display(const struct SwitchSPDT sw) {
  ...
  uint8_t state = bossy.savedState(sw.id);

  uint8_t color[3] = {sw_first_colors[state], sw_second_colors[state], sw_third_colors[state]};
  uint8_t x[3] = {sw.first_pixel[Xi], sw.second_pixel[Xi], sw.third_pixel[Xi]};
  uint8_t y[3] = {sw.first_pixel[Yi], sw.second_pixel[Yi], sw.third_pixel[Yi]};
...
void update_pot_display(uint8_t pot_id, uint8_t input_id) {
  uint16_t reading = bossy.savedReading(input_id);
  uint16_t blocks = min((reading - 100) / 100, 8);  
...
void toggle_mode_if_commanded() {
  ...
  bool is_stick_mode = bossy.savedState(LEFT_SWITCH_UPPER) == LEFT;
  bool toggled = false;
